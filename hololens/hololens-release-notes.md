...
- In een statusbericht op de pagina Instellingen wordt het aantal bestanden weergegeven dat in afwachting is van uploaden (of 'OneDrive is up-to-date' wanneer alle bestanden in behandeling zijn geüpload).
...
- Herstelt grote beschadiging van video's met color-kalibratie.
...
De nieuwe Microsoft Edge [maakt](https://blogs.windows.com/windowsexperience/2018/12/06/microsoft-edge-making-the-web-better-through-more-open-source-collaboration/) gebruik van het Chromium open source project voor betere compatibiliteit voor klanten en minder fragmentatie van het web voor webontwikkelaars.
...
> [!NOTE]
> Wanneer u de nieuwe Microsoft Edge op HoloLens 2, worden uw instellingen en gegevens geïmporteerd uit verouderde Microsoft Edge. Als u verouderde Microsoft Edge na het starten van de nieuwe Microsoft Edge blijft gebruiken, worden die nieuwe gegevens niet gesynchroniseerd van verouderde Microsoft Edge naar de nieuwe Microsoft Edge.
...
- System > Sound:
  - Audioapparaten voor invoer en uitvoer: kies onafhankelijk van elkaar uw invoer- en uitvoer audioapparaten (u kunt bijvoorbeeld luisteren naar audio via Bluetooth-microfoons of een USB-C-microfoon gebruiken voor audio-invoer).
    > [!NOTE]
...
#### <a name="display-color-calibration"></a>Kleurdeseratie weergeven
...
![Stel de standaardinstelling voor de microfoon in.](images/usbc-mic-3.jpg)
...
We breiden de lijst met beschikbare URI's uit Instellingen die IT-beheerders kunnen beheren. Sommige van deze URI's zijn voor nieuw beschikbare gebieden binnen de nieuwe Instellingen app. Als u het beleid Instellingen/PageVisibilityList gebruikt, bekijkt u de volgende lijst en past u de toegestane of geblokkeerde pagina's zo nodig aan.
...
✔️ fout [**bij het in de kioskmodus geven verbeterd:**](#kiosk-mode-behavior-changes-for-handling-of-failures)
...
Meer informatie over [het automatisch starten van inrichting via een USB](hololens-provisioning.md#auto-launch-provisioning-from-usb) vindt u in HoloLens inrichtingsdocumentatie.
...
[TenantLockdown](/windows/client-management/mdm/tenantlockdown-csp) Met CSP HoloLens 2 alleen gekoppeld aan MDM-inschrijving met behulp van Autopilot. Zodra het knooppunt RequireNetworkInOOBE van tenantLockdown CSP is ingesteld op waar of onwaar (in eerste instantie ingesteld) op HoloLens 2, blijft die waarde op het apparaat staan ondanks het opnieuw knipperen, updates van het besturingssysteem, enzovoort. 
...
- Deze release bevat de NCSI-proxydetectiefix voor het oplossen van mislukte internetdetectie via een netwerkproxy. NCSI kan machineproxy en proxy per profiel gebruiken voor detectie van internetverbinding. Proxy per gebruiker wordt ondersteund door NCSI in toekomstige release.
- Op de Windows Mixed Reality apparaten is de vector van de voorwaartse richting parallel aan de grond wanneer het hoofd van de gebruiker zich in een neutrale positie naar voren kijkt. Eerdere versies van HoloLens 2 echter de vector uitgelijnd om in plaats daarvan haaks te staan op de weergavepaneel, die een paar graden omlaag wordt gekanteld ten opzichte van de ideale stand. Nieuwere versies van HoloLens 2 hebben dit gecorrigeerd om semantische consistentie tussen formulierfactoren te garanderen.
...
- Er is een probleem opgelost met herhaalde storedownloads voor apps die al actueel zijn.
...
Een [CSP (Configuration Service Provider)](/windows/client-management/mdm/configuration-service-provider-reference?redirectedfrom=MSDN) is een interface voor het lezen, instellen, wijzigen of verwijderen van configuratie-instellingen op een apparaat. In deze release voegen we ondersteuning toe voor meer beleid om de controle te vergroten die beheerders hebben over geïmplementeerde HoloLens apparaten. Zie NetworkQoSPolicy CSP voor HoloLens lijst met [CSP's die door CSP's worden ondersteund.](/windows/client-management/mdm/networkqospolicy-csp)
...
- Oplossingen en verbeteringen voor naleving van OpenXR-runtime voor HoloLens 2 en de emulator.
