...
> [!IMPORTANT]
> Vanwege het grote aantal browserbeleidsregels dat door de nieuwe Microsoft Edge wordt ondersteund, kan ons team niet garanderen dat elk nieuw beleid op de HoloLens 2. We hebben echter getest en bevestigd dat het nieuwe Microsoft Edge-equivalent van elk verouderd Microsoft Edge-beleid dat eerder werd ondersteund op HoloLens 2 werken zoals verwacht. Zie [Microsoft Edge (oudere versie)](/deployedge/microsoft-edge-policy-map-legacy-to-newedge) Microsoft Edge toewijzen van beleid om het nieuwe Microsoft Edge-equivalent te vinden van elk verouderd Microsoft Edge-browserbeleid dat u gebruikte met HoloLens 2.
>
...
- System > Sound:
  - Audioapparaten voor invoer en uitvoer: kies onafhankelijk uw invoer- en uitvoer audioapparaten (bijvoorbeeld luisteren naar audio via Bluetooth-microfoons of gebruik een USB-C-microfoon voor audio-invoer).
    > [!NOTE]
...
Vanaf deze Windows versie wordt de kioskervaring terugvallen op de algemene kioskconfiguratie (indien aanwezig) in het geval van fouten tijdens de kioskmodus van de AAD-groep.
...
| Privacy-> Ink-& het typen van persoonlijke instellingen             | `ms-settings:privacy-speechtyping`                 |
...
✔️ de [**verbeterde fout in de kioskmodus:**](#kiosk-mode-behavior-changes-for-handling-of-failures)
...
1.  Selecteer op HoloLens 2 apparaat Het menu Start openen, selecteer Alle apps start de Verkenner-app.
...
Certificaatinstallatie ondersteunt momenteel CER- en CRT-bestanden. Apparaateigenaren kunnen certificaten installeren op de lokale computer en de huidige gebruiker;  alle andere gebruikers kunnen alleen installeren in Huidige gebruiker. Gebruikers kunnen alleen rechtstreeks geïnstalleerde certificaten uit de Instellingen verwijderen. Als een certificaat op een andere manier is geïnstalleerd, moet het ook door hetzelfde mechanisme worden verwijderd.
...
Vóór deze release moesten gebruikers het inrichtingsscherm handmatig starten tijdens OOBE om in te delen met behulp van een knopcombinatie. Gebruikers kunnen nu de knopcombinatie overslaan met behulp van een inrichtingspakket op een USB-opslagstation. 
...
1. Wanneer het HoloLens 2 wordt opstart in OOBE, wordt het inrichtingspakket op het USB-station automatisch gedetecteerd en wordt de inrichtingspagina start.
...
[TenantLockdown](/windows/client-management/mdm/tenantlockdown-csp) Met CSP HoloLens 2 alleen gekoppeld aan MDM-inschrijving met behulp van Autopilot. Zodra het CSP-knooppunt RequireNetworkInOOBE van TenantLockdown is ingesteld op waar of onwaar (in eerste instantie ingesteld) op HoloLens 2, blijft die waarde op het apparaat staan ondanks dat het apparaat opnieuw is geflitst, besturingssysteemupdates, enzovoort. 

Zodra het knooppunt RequireNetworkInOOBE van de TenantLockdown-CSP's is ingesteld op true op HoloLens 2, wacht OOBE voor onbepaalde tijd totdat het Autopilot-profiel is gedownload en toegepast, na de netwerkverbinding. 
...
1. Maak het HoloLens 2 apparaat lid van de groep die u in de vorige stap hebt gemaakt en synchronisatie activeren.  
...
Met deze nieuwe functie kan een IT-beheerder een HoloLens 2-apparaat configureren voor de kioskmodus voor meerdere apps die van toepassing is op systeemniveau, geen affiniteit heeft met een identiteit op het systeem en van toepassing is op iedereen die zich bij het apparaat heeft meldt. Meer informatie over deze nieuwe functie wordt uitgebreid beschreven in [HoloLens kioskmodus](hololens-kiosk.md).
...
> [!NOTE]
> Voor consistente ervaring met HoloLens 2, moet u ervoor zorgen dat waarden voor zowel DisplayOffTimeoutOnBattery als StandbyTimeoutOnBattery als dezelfde waarde zijn ingesteld. Hetzelfde geldt voor DisplayOffTimeoutPluggedIn en StandbyTimeoutPluggedIn. Raadpleeg [Inactieve timers weergeven, slaapstand](/windows-hardware/design/device-experiences/display--sleep--and-hibernate-idle-timers) en sluimerstand voor meer informatie over moderne stand-by.
...
-   Naast sensoren die worden blootgesteld in HoloLens onderzoeksmodus (eerste generatie), bieden we nu IMU-sensortoegang, waaronder een versnellingsmeter,scope en teller.
...
Deze maandelijkse kwaliteitsupdate bevat geen belangrijke wijzigingen. We raden u aan onze nieuwste build van de functiere release uit te Windows Holographic versie 20H2.
...
- Er is een probleem opgelost dat ertoe kan leiden dat hologrammen verdwijnen in Unity-toepassingen wanneer ze verliezen of weer worden bijgezet.
- Er is een probleem opgelost waardoor exclusieve HoloLens terug naar de shell zijn gecrasht tijdens het gebruik van de HoloLens Emulator hardwareversnelling op bepaalde apparaten.
...
- Er is een probleem opgelost waardoor HoloLens apps die de pixelindeling wijzigen, zwart in de HoloLens emulator.
...
- Verbeterde stabiliteit van hologrammen in mixed reality vastleggen wanneer het *HolographicDepthReprojectionMethod AutoPlanar-algoritme* wordt gebruikt.
