...
|-------------------------|----------------------------|--------------|------------------|
| [Platformmodus verplaatsen](#moving-platform-mode) | Introduceert de bètaversie van de Moving Platform-modus, die, wanneer deze is geconfigureerd, het gebruik van HoloLens 2 op grote zeepaden met een laag dynamische beweging mogelijk maakt. | Alles | 20348.1411 |
...
✔️ Als u de apps van uw organisatie beschikbaar wilt maken in de bedrijfs store via de Microsoft Store, maar alleen toegang wilt toestaan tot de apps van uw organisatie en niet de volledige Store, stelt u dit beleid [in.](#use-only-private-store-apps-for-microsoft-store) <br>
...
Met dit nieuwe beleid AutoLogonUser bepaalt u of een gebruiker automatisch wordt aangemeld. Sommige klanten willen apparaten instellen die zijn gekoppeld aan een identiteit, maar geen aanmeldingservaring willen. Imagine apparaat ophalen en direct hulp op afstand gebruiken. Of u hebt het voordeel dat u uw apparaten snel HoloLens distribueert en hun eindgebruikers in staat stelt om zich sneller aan te melden.
...
- Een TOEGEWEZEN [toegangs-API](/uwp/api/windows.system.userprofile.assignedaccesssettings?view=winrt-20348) ingeschakeld zodat apps nu kunnen bepalen of een HoloLens wordt uitgevoerd in de kioskmodus voor de gebruiker die is aangemeld bij de HoloLens.
...
Selecteer **Bevestigen > Nu opnieuw opstarten om** te voltooien. Nadat uw apparaat opnieuw is opgestart, gaat u naar **Instellingen > Update & Security > Controleren** op updates om de meest recente build op te halen.
...
Controleer als HoloLens een productie-build wordt uitgevoerd:
