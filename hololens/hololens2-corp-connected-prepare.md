...
description: Meer informatie over het voorbereiden van de registratie HoloLens 2 apparaten via een bedrijfsnetwerk dat is verbonden met Dynamics 365 Guides.
...
- Werknemers gebruiken hun Azure AD-account om het apparaat te registreren in Azure AD en kunnen het automatisch inschrijven bij de MDM-oplossing van de organisatie (Azure AD + MDM vereist een [Azure AD Premium-abonnement).](/azure/active-directory/fundamentals/active-directory-get-started-premium)
...
Microsoft Intune, onderdeel van Enterprise Mobility + Security, is een cloud-gebaseerd MDM-systeem dat apparaten beheert die zijn verbonden met uw tenant. Net Office 365 gebruikt Intune Azure AD voor identiteitsbeheer, zodat werknemers dezelfde referenties gebruiken om apparaten in te schrijven bij Intune die ze gebruiken om zich aan te melden bij Office 365. Intune ondersteunt ook apparaten met andere besturingssystemen, zoals iOS en Android, om een volledige MDM-oplossing te bieden. In deze handleiding richten we ons op het gebruik van Intune voor het inschakelen van een implementatie naar uw interne netwerk met HoloLens 2.
> [!Important] 
...
Certificaten helpen de beveiliging te verbeteren door accountverificatie, Wi-Fi, VPN-versleuteling en SSL-versleuteling van webinhoud. Hoewel beheerders certificaten op apparaten handmatig kunnen beheren via inrichtingspakketten, is het een best practice om uw MDM-systeem te gebruiken om deze certificaten gedurende de hele levenscyclus te beheren, van inschrijving tot verlenging en intrekking. 
...
Als uw MDM-systemen al zijn geconfigureerd voor certificaten, verwijst u naar Certificaten en netwerkprofielen voorbereiden voor [HoloLens 2](/hololens/hololens-certificates-network) om te beginnen met het implementeren van certificaten en profielen voor uw HoloLens 2 apparaten.
...
Als uw netwerk scEP nog niet ondersteunt of als u niet zeker weet of uw netwerk correct is ingesteld voor SCEP met Intune, verwijst u naar Infrastructuur configureren om SCEP met [Intune te ondersteunen.](/mem/intune/protect/certificates-scep-configure)

Als uw infrastructuur scEP al ondersteunt, [](/mem/configmgr/protect/deploy-use/create-certificate-profiles) moet u een profiel maken voor elk SCEP-certificaat dat door de HoloLens 2 gebruikt. [](/mem/intune/protect/certificates-profile-scep) Als u problemen hebt met SCEP, gebruikt u Problemen met het gebruik van [SCEP-certificaatprofielen](/troubleshoot/mem/intune/troubleshoot-scep-certificate-profiles)oplossen om certificaten in te Microsoft Intune .

## <a name="pkcs"></a>PKCS
Intune ondersteunt ook het gebruik van persoonlijke en PKCS-certificaten (public key pair). Verwijzing [Persoonlijke en openbare-sleutelcertificaten gebruiken in Microsoft Intune](/mem/intune/protect/certificates-pfx-configure) voor meer informatie.
...
Er zijn een aantal verschillende typen proxy's en manieren om proxy te configureren. Voor deze handleiding kiezen we voor **Wi-Fi-proxy, instellen via PAC-URL** en geïmplementeerd via MDM. Dit heeft de voordelen van automatisch via MDM worden geïmplementeerd, het PAC-bestand kunnen bijwerken in plaats van een server:poortconfiguratie te gebruiken en ten slotte de Wi-Fi-proxy te gebruiken om een proxy zo te configureren dat deze alleen van toepassing is op één Wi-Fi-verbinding, zodat de apparaten nog steeds kunnen worden gebruikt als ze op een andere locatie zijn verbonden.

Zie Create a Wi-Fi profile for devices in Windows 10 Microsoft Intune - Azure (Een Wi-Fi maken voor apparaten in Azure) voor meer informatie over [proxy-instellingen voor apparaten.](/mem/intune/configuration/wi-fi-settings-configure)
...
Er zijn meerdere manieren om toepassingen te implementeren op HoloLens 2 apparaten. Apps kunnen rechtstreeks worden geïmplementeerd via MDM, de Microsoft Store voor Bedrijven (MSfB) of sideloaden via een inrichtingspakket. Omwille van deze handleiding implementeren we apps via MDM, door gebruik te maken van de vereiste app-installatie. Hierdoor kunnen uw LOB-apps automatisch worden gedownload naar uw HoloLens zodra ze zijn ingeschreven.
...
Meer informatie over app-implementatie vindt u op [App Management: Overzicht.](/hololens/app-deploy-overview)

> [!NOTE]
> HoloLens 2 ondersteunt alleen het uitvoeren van UWP ARM64-apps.

## <a name="guides-playbook"></a>Playbook voor handleidingen
Gidsen maken gebruik van een Microsoft Dataverse-omgeving als gegevensstore voor uw Guides-apps. Het is belangrijk om een beter beeld te krijgen van de manier waarop uw Dataverse-omgeving communiceert met uw Guides-apps en uw tenant. In deze handleiding gaan we niet in op het beheren van uw gegevensverse, maar raadpleegt u Basisconcepten voor het implementeren van [Dynamics 365 Guides - Dynamics 365 Mixed Reality](/dynamics365/mixed-reality/guides/admin-deployment-playbook).
