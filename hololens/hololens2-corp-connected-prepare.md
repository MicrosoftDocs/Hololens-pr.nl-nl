...
Microsoft Intune, onderdeel van Enterprise Mobility + Security, is een cloudgebaseerd MDM-systeem dat apparaten beheert die zijn verbonden met uw tenant. Net als Office 365 gebruikt Intune Azure AD voor identiteitsbeheer, zodat werknemers dezelfde referenties gebruiken om apparaten in te schrijven bij Intune die ze gebruiken om zich aan te melden bij Office 365. Intune ondersteunt ook apparaten met andere besturingssystemen, zoals iOS en Android, om een volledige MDM-oplossing te bieden. Voor deze handleiding richten we ons op het gebruik van Intune voor het inschakelen van een implementatie in uw interne netwerk met HoloLens 2.
> [!Important] 
...
Als uw MDM-systemen al zijn geconfigureerd voor certificaten, verwijst u naar Certificaten en [netwerkprofielen](/hololens/hololens-certificates-network) voorbereiden voor HoloLens 2 om te beginnen met het implementeren van certificaten en profielen voor uw HoloLens 2 apparaten.
...
Als uw netwerk nog geen ondersteuning biedt voor SCEP of als u niet zeker weet of uw netwerk juist is ingesteld voor SCEP met Intune, verwijst u naar Infrastructuur configureren om SCEP met  [Intune](/mem/intune/protect/certificates-scep-configure)te ondersteunen.
...
## <a name="pkcs"></a>PKCS
Intune ondersteunt ook het gebruik van persoonlijke en PKCS-certificaten (public key pair). Referentie [Gebruik persoonlijke en openbare-sleutelcertificaten in Microsoft Intune](/mem/intune/protect/certificates-pfx-configure) voor meer informatie.

## <a name="proxy"></a>Proxy
De meeste bedrijfs intranetnetwerken maken gebruik van een proxy voor het beheren van extern verkeer. Met HoloLens 2 kunt u een proxyserver configureren voor ethernetWi-Fi en VPN-verbindingen.

Er zijn een aantal verschillende typen proxy's en manieren om proxy te configureren. In deze handleiding kiezen we voor **Wi-Fi-proxy, instellen via PAC-URL** en geïmplementeerd via MDM. Dit heeft de voordelen van automatisch worden geïmplementeerd via MDM, de mogelijkheid om het PAC-bestand bij te werken in plaats van een server:poortconfiguratie te gebruiken en ten slotte Wi-Fi proxy te gebruiken om een proxy te configureren zodat deze alleen van toepassing is op één Wi-Fi-verbinding, zodat de apparaten nog steeds kunnen worden gebruikt als ze op een andere locatie zijn verbonden.

Zie Create a Wi-Fi profile for devices in Windows 10 Microsoft Intune - Azure (Een Wi-Fi maken voor apparaten in azure) voor meer informatie over proxy-instellingen [voor Microsoft Intune.](/mem/intune/configuration/wi-fi-settings-configure)
