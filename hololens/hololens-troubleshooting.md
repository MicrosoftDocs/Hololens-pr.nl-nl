...
**Bekende problemen**
- [Insider-oplossing: telkens wanneer de stroom naar 18 procent gaat, wordt het apparaat plotseling automatisch afgesloten](#every-time-the-power-goes-to-18-percent-the-device-suddenly-shuts-down-automatically)
...
> [!NOTE]
> Er is een oplossing voor dit probleem beschikbaar in [Windows Insiders.](hololens-insider.md)
...
Als u OneDrive For Business gebruikt met uw Azure AD-account, is er mogelijk een fout opgetreden bij het aanmelden bij uw postvak IN OneDrive app. Het niet kunnen aanmelden bij de app OneDrive heeft geen invloed op het automatisch uploaden van afbeeldingen en video's die zijn vastgelegd door de Camera-app. Uw bestanden kunnen nog steeds worden opgeslagen en gebruikt vanuit OneDrive voor Bedrijven cloudopslag. De OneDrive- en HoloLens-teams werken aan het probleem.

### <a name="workarounds"></a>Tijdelijke oplossingen

Vereiste: klanten kunnen een Microsoft Edge en het besturingssysteem van het apparaat wordt bijgewerkt naar een Windows Holographic- of 21H1-build of nieuwer.

Als u dit probleem ondervindt, probeert u een van de volgende opties:

- Gebruikers hebben rechtstreeks toegang OneDrive For Business vanuit Microsoft Edge en communiceren met hun bestanden op de website vanuit hun browser.
...
Deze fout tkomt doorgaans aan tijdens het opnieuw instellen van het apparaat en het opnieuw gebruiken van stromen waarbij een HoloLens apparaat ten minste één keer via Autopilot is gegaan. Als u dit probleem wilt oplossen, verwijdert [u het](/mem/autopilot/troubleshoot-device-enrollment#error-code-0x80180014-when-re-enrolling-using-self-deployment-or-pre-provisioning-mode) apparaat Microsoft Intune en stelt u het opnieuw in om de Autopilot-stroom te voltooien.

Raadpleeg de stappen voor probleemoplossing [op de Autopilot-pagina voor meer informatie.](hololens2-autopilot.md#issue---mdm-enrollment-fails-with-error-0x80180014-error-code-during-autopilot)

## <a name="microsoft-store-error-code-0x80131500"></a>Microsoft Store foutcode 0x80131500

Sommige gebruikers ervaren mogelijk dat de Microsoft Store niet werkt zoals verwacht, en zien de foutcode 0x80131500. Dit is een probleem dat wordt veroorzaakt doordat de regio die is ingesteld op de HoloLens niet beschikbaar is in Microsoft Store app op HoloLens. Als u foutcodefouten 0x80131500, kunt u het volgende doen:
...
1. Als u wilt dat het hele apparaat de wijziging weerspiegelt, moet het apparaat opnieuw worden opgestart.

Het HoloLens werkt aan het toevoegen van ondersteuning voor meer regio's.
...
Wanneer gebruikers Microsoft Edge de microfoon niet starten, waardoor ze niet kunnen communiceren met Edge in HoloLens. Dit bekende probleem heeft betrekking op de versie van de Microsoft Edge-app. Gebruik geen eerdere reflash op uw apparaat, omdat dit probleem hiermee niet wordt opgelost.

### <a name="who-is-affected"></a>Wie wordt beïnvloed?

Gebruikers met Microsoft Edge versie 93, 94 of 95.
U kunt controleren welke versie van Microsoft Edge u hebt met behulp van de Microsoft Store-app en vervolgens de knop Meer weergeven selecteren die wordt vertegenwoordigd door **...** en vervolgens Downloads en **updates selecteren.**

### <a name="work-around"></a>Work around

De huidige oplossing is in versie 96, die beschikbaar is voor gebruikers die zich hebben geregistreerd bij Microsoft Edge Insiders. Dit is anders dan het registreren van uw apparaat als Windows Insider. Lees deze instructies voor meer informatie over het inschrijven bij het [insider-programma van Edge.](hololens-new-edge.md#microsoft-edge-insider-channels)
...
Als u een apparaat snel opnieuw wilt opstarten op Windows Holographic, opent u versie [21H1](hololens-release-notes.md#windows-holographic-version-21h1) het menu Start, selecteert u het gebruikerspictogram en selecteert u opnieuw **opstarten.**
...
Er is een probleem tijdens OOBE, waarbij wanneer de gebruiker een werk- of schoolaccount heeft gekozen en zijn of haar wachtwoord heeft invoeren, het overschakelen naar de speciale tekens op het toetsenbord door op de knop &123 te tikken, niet wordt gewijzigd in speciale tekens. Dit is een **bekend probleem.**
...
In eerdere builds van Windows Holographic zou het resultaat een HTTP-foutpagina zijn wanneer u probeert een vergrendeld bestand te downloaden. In Windows Holographic, versie 21H1-update, wordt er niets weergegeven wanneer u probeert een vergrendeld bestand te downloaden. Het bestand wordt niet gedownload en er is geen fout.

[Terug naar lijst](#list)

## <a name="device-portal-file-uploaddownload-times-out"></a>Apparaatportal voor uploaden/downloaden van bestanden
> [!NOTE]
...
Deze tijdelijke oplossing, die ook van toepassing is op Wi-Fi en UsbNcm, is om de optie 'vereist' onder 'SSL-verbinding' uit te schakelen. Als u dit wilt doen, gaat u Apparaatportal systeem **en** selecteert u de **pagina Voorkeuren.** Zoek in **de sectie Apparaatbeveiliging** de **optie SSL-verbinding** en schakel vereist **uit.**
...
## <a name="cant-sign-in-because-my-hololens-was-previously-set-up-for-someone-else"></a>Kan niet aanmelden omdat mijn HoloLens is ingesteld voor iemand anders
...
Als uw HoloLens 2 niet op uw stem reageert, moet u ervoor zorgen dat Spraakherkenning is ingeschakeld. Ga naar **Start**  >  **Instellingen**  >  **Privacy**  >  **Speech** en schakel **Spraakherkenning in.**

[Terug naar lijst](#list)

## <a name="hand-input-isnt-working"></a>Handinvoer werkt niet

Om ervoor te HoloLens dat u uw handen kunt zien, moet u ze in het bewegingsframe houden.  De Mixed Reality Start geeft feedback waarmee u kunt zien wanneer uw handen worden bijgespoord.  De feedback verschilt op verschillende versies van HoloLens:
