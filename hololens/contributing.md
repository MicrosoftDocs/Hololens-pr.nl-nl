---
title: Bijdragen aan instructies
...
HoloLens documenten worden weergegeven op het docs.microsoft.com-platform, dat gebruikmaakt GitHub markdown met Markdig-functies. De inhoud die u in deze repo bewerkt, wordt opgemaakt in niet-geautoriseerde pagina's die worden weergegeven op /hololens.
...
Bij het instellen van GitHub account raden we u ook deze voorzorgsmaatregelen aan:
...
Gebruik de volgende werkstroom om updates aan te brengen *in een bestaand* artikel via GitHub in een webbrowser:
...
2. Selecteer rechtsboven de knop Bewerken (potloodpictogram).
   ...
   Hiermee wordt automatisch een vertakking voor eenmalig gebruik van de standaard vertakking gevorkd, _master_.
...
6. Selecteer op de volgende pagina Pull-aanvraag **maken om** de automatisch gemaakte vertakking samen te voegen in de standaard branch, _master_.
...
- De `redirect_url` is de relatieve openbare URL van het oude artikel naar het nieuwe artikel. Zorg ervoor dat deze URL **geen** of bevat, omdat deze verwijst naar de openbare `mixed-reality-docs` URL en niet naar het pad naar de `.md` opslagplaats. Het is toegestaan om een koppeling te maken naar een sectie in het nieuwe artikel `#section` met behulp van . U kunt hier indien nodig ook een absoluut pad naar een andere site gebruiken.
...
8. Wanneer u klaar bent, **selecteert u Commit new file**.
...
2. Maak een lokale kloon van uw fork in Visual Studio Code:
    ...
    5. Selecteer **Open repo** in het pop-up.
...
1. Zorg ervoor dat de gekloonde fork is bijgewerkt met de officiële repo.
   ...
   1. Maak in een webbrowser een pull-aanvraag om recente wijzigingen van andere inzenders in de standaardvertakking van MicrosoftDocs/mixed-reality, _master,_ te synchroniseren met uw fork (zorg ervoor dat de pijl naar de juiste bestemming wijst).
...
2. Maak of bewerk artikelen in uw gekloonde repo met behulp Visual Studio Code.
   ...
   3. **Alle wijzigingen** in **broncodebeheer (schrijf** het commit-bericht wanneer hier om wordt gevraagd) door.
      ...
      ![Kies Alles door te geven in Broncodebeheer](images/source-control-commit.png)
