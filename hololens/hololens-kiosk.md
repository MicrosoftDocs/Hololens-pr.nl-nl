...
1. **Wie gebruikt uw kiosk en welk [type account(s)](hololens-identity.md) gebruiken ze?** Dit is een beslissing die u waarschijnlijk al hebt genomen en niet moet worden aangepast met het oog op uw Kiosk, maar van invloed is op hoe de kiosk later wordt toegewezen.
...
-   U maakt meerdere Kiosk-profielen en wijst deze toe aan verschillende gebruikers/groepen. Zoals een kiosk voor uw Azure AD-groep die veel apps heeft en een bezoekers die een kiosk met meerdere apps met één app heeft.
-   Uw kioskconfiguratie wordt een **profiel-id genoemd** en heeft een GUID.
-   U wijst dat profiel toe in de sectie configuraties door het gebruikerstype op te geven en dezelfde GUID voor **de DefaultProfile-id te gebruiken.**
...
|Implementeren met behulp van Azure Active Directory (Azure AD)  | Niet vereist            | Niet vereist                   | Vereist  |
...
1. [Bereid u voor om de apparaten in te schrijven.](#mdmenroll)
...
U kunt uw MDM-systeem configureren om HoloLens automatisch in te schrijven wanneer de gebruiker zich voor het eerst heeft aanmelden of om gebruikers handmatig apparaten te laten registreren. De apparaten moeten ook worden samengevoegd met uw Azure AD-domein en worden toegewezen aan de juiste groepen.

Zie [Enroll HoloLens in MDM](hololens-enroll-mdm.md) and [Intune enrollment methods for Windows devices](/mem/intune/enrollment/windows-enrollment-methods)(Inschrijvingsmethoden in MDM en Intune voor Windows apparaten) voor meer informatie over het inschrijven van de apparaten.
...
1. Selecteer **Platform**  >  **Windows 10 en hoger** en selecteer vervolgens **Profieltype**  > **Apparaatbeperkingen.**
...
1. Selecteer Toevoegen om te beginnen met het configureren **van de** kiosk.
...
Zie Apparaatinstellingen voor Windows 10 en Windows Holographic for Business uitvoeren als toegewezen kiosk met Intune voor meer informatie over het maken van een [kioskconfiguratieprofiel.](/intune/configuration/kiosk-settings)

### <a name="mdm-step-3-single-app-ndash--configure-the-settings-for-a-single-app-kiosk"></a><a id="mdmconfigsingle"></a>MDM, stap 3 (één app) De instellingen voor een kiosk voor één &ndash;  app configureren

In deze sectie worden de instellingen samengevat die een kiosk voor één app vereist. Zie de volgende artikelen voor meer informatie:

- Zie Kioskmodus configureren met behulp van Microsoft Intune voor meer informatie over het configureren van een [kioskconfiguratieprofiel](hololens-commercial-infrastructure.md#how-to-configure-kiosk-mode-using-microsoft-intune)in Intune.
- Zie Kiosken voor apps met één volledig scherm voor meer informatie over de beschikbare instellingen voor [kiosken](/intune/configuration/kiosk-settings-holographic#single-full-screen-app-kiosks) voor één app in Intune
- Voor andere MDM-services raadpleegt u de documentatie van uw provider voor instructies. Als u een aangepaste XML-configuratie moet gebruiken om een kiosk in te stellen in uw MDM-service, maakt u een XML-bestand dat [de kioskconfiguratie definieert.](#ppkioskconfig)

1. Selecteer **Gebruikerslogotype** Lokaal gebruikersaccount en voer vervolgens de gebruikersnaam in van het lokale  >  (apparaat)account of microsoft-account (MSA) dat zich kan aanmelden bij de kiosk.
   > [!NOTE]  
...
### <a name="mdm-step-3-multi-app-ndash-configure-the-settings-for-a-multi-app-kiosk"></a><a id="mdmconfigmulti"></a>MDM, stap 3 (meerdere apps) De instellingen voor een kiosk voor meerdere &ndash; apps configureren

In deze sectie worden de instellingen samengevat die een kiosk voor meerdere apps vereist. Raadpleeg de volgende artikelen voor meer informatie:
...
- Voor andere MDM-services raadpleegt u de documentatie van uw provider voor instructies. Als u een aangepaste XML-configuratie moet gebruiken om een kiosk in te stellen in uw MDM-service, maakt u een XML-bestand dat [de kioskconfiguratie definieert.](#ppkioskconfig) Als u een XML-bestand gebruikt, moet u de [indeling Start opnemen.](#start-layout-for-hololens)  
- U kunt eventueel een aangepaste Indeling voor start gebruiken met Intune of andere MDM-services. Zie Indelingsbestand starten voor [MDM (Intune en meer)](#start-layout-file-for-mdm-intune-and-others)voor meer informatie.

1. Selecteer **Doel Windows 10 in S-modus apparaten**  >  **Nee.**  
>[!NOTE]  
...
1. Selecteer **Aanmeldingstype Gebruiker**  >  **Azure AD-gebruiker** of -groep of Aanmeldingstype gebruiker HoloLens bezoekers en voeg vervolgens een of meer gebruikersgroepen of   >  -accounts toe.  
   ...
   Alleen gebruikers die deel uitmaken van de groepen of accounts die u opgeeft in **Aanmeldingstype** gebruiker, kunnen de kioskervaring gebruiken.

1. Selecteer een of meer apps met behulp van de volgende opties:
   - Als u een geüploade Line-Of-Business-app wilt toevoegen, selecteert u **Store-app toevoegen** en selecteert u vervolgens de app die u wilt.
   - Als u een app wilt toevoegen door de AUMID op te geven, selecteert u **Toevoegen door AUMID** en voert u vervolgens de AUMID van de app in. [Bekijk de lijst met beschikbare AUMID's](#aumids)
...
Wanneer u een MDM-systeem gebruikt, kunt u het apparaat tijdens OOBE inschrijven bij MDM. Nadat OOBE is klaar, is het eenvoudig om u aan te melden bij het apparaat.
...
1. Wacht tot OOBE is uitgevoerd, totdat de Store-app is gedownload en geïnstalleerd en totdat beleid is toegepast. Start het apparaat vervolgens opnieuw op.

De volgende keer dat u zich bij het apparaat aanmelden, wordt de kiosk-app automatisch start.
...
Wanneer u een MDM-systeem gebruikt, kunt u het apparaat aan uw Azure AD-tenant deelnemen en het apparaat tijdens OOBE inschrijven bij MDM. Geef, indien van toepassing, de inschrijvingsgegevens op aan de gebruikers, zodat deze beschikbaar zijn tijdens het OOBE-proces.

> [!NOTE]  
> Als u het kioskconfiguratieprofiel hebt toegewezen aan een groep die gebruikers bevat, moet u ervoor zorgen dat een van deze gebruikersaccounts het eerste account is om u aan te melden bij het apparaat.

Volg deze stappen tijdens OOBE:

1. Meld u aan met het account dat deel uitmaken van de **groep Aanmeldingstype** gebruiker.
...
1. Nadat OOBE is klaar, kunt u extra apps installeren vanuit de Microsoft Store of door sideloaden. [Vereiste apps](/mem/intune/apps/apps-deploy#assign-an-app) voor de groep waar het apparaat bij hoort, worden automatisch geïnstalleerd.
1. Nadat de installatie is voltooien, start u het apparaat opnieuw op.

De volgende keer dat u zich bij het apparaat aan melden met een account dat bij het **aanmeldingstype** Gebruiker hoort, wordt de kiosk-app automatisch start.
...
### <a name="provisioning-package-step-1-ndash-create-a-kiosk-configuration-xml-file"></a><a id="ppkioskconfig"></a>Inrichtingspakket, stap 1 &ndash; Een XML-bestand voor kioskconfiguratie maken
...
- Gebruik de [tijdelijke aanduiding Beginindeling XML voor](#start-layout-for-hololens) HoloLens.
...
In de [ **sectie Configuraties van** het XML-bestand](/windows/configuration/lock-down-windows-10-to-specific-apps#configs)kunt u een speciale groep met de naam **Bezoekers** configureren zodat gasten de kiosk kunnen gebruiken. Wanneer de kiosk is geconfigureerd ter ondersteuning van de speciale **groep Bezoekers,** wordt de optie Gast **toegevoegd** aan de aanmeldingspagina. Voor **het** gastaccount is geen wachtwoord vereist en alle gegevens die aan het account zijn gekoppeld, worden verwijderd wanneer het account wordt afgeschreven.
...
1. Maak een inrichtingspakket dat:
    ...
    1. Het apparaat kan eventueel worden ingeschreven bij MDM (Runtime-instellingen/Werkplek/Registraties), zodat het later kan worden beheerd.
...
#### <a name="placeholder-start-layout-for-hololens"></a><a id="start-layout-for-hololens"></a>Beginindeling van tijdelijke aanduiding voor HoloLens
...
> [!NOTE]  
> Omdat een kiosk voor één app de kiosk-app start wanneer een gebruiker zich meldt, gebruikt deze geen Startmenu en hoeft deze geen indeling Start te hebben.

> [!NOTE]  
> Als u [MDM gebruikt om](#use-microsoft-intune-or-other-mdm-to-set-up-a-single-app-or-multi-app-kiosk) een kiosk voor meerdere apps in te stellen, kunt u eventueel een indeling Start gebruiken. Zie Tijdelijke aanduiding Indelingsbestand [voor start voor MDM (Intune en andere)](#start-layout-file-for-mdm-intune-and-others)voor meer informatie.
...
> [!NOTE]
> Als u een aangepaste instelling en een volledige XML-configuratie moet gebruiken om een kiosk in te stellen in uw MDM-service, gebruikt u de instructies voor de indeling Start voor [een inrichtingspakket.](#start-layout-for-hololens)
...
1. Selecteer **Geavanceerde inrichting,** voer een naam in voor uw project en selecteer **volgende.**
...
1. **Optioneel**. (Als u het inrichtingspakket wilt toepassen na de eerste installatie van het apparaat en er al een gebruiker met beheerdersrechten beschikbaar is op het kioskapparaat, slaat u deze stap over.) Selecteer **Runtime-instellingen** &gt;  &gt; **AccountsGebruikers** en maak vervolgens een gebruikersaccount. Geef een gebruikersnaam en wachtwoord op en selecteer **vervolgens UserGroup**  >  **Administrators.**  
  
     Met dit account kunt u de inrichtingsstatus en logboeken bekijken.  
1. **Optioneel**. (Als u al een niet-beheerdersaccount op het kioskapparaat hebt, slaat u deze stap over.) Selecteer **Runtime-instellingen** &gt;  &gt; **AccountsGebruikers** en maak vervolgens een lokaal gebruikersaccount. Zorg ervoor dat de gebruikersnaam hetzelfde is als voor het account dat u in de configuratie-XML opgeeft. Selecteer **UserGroup**  >  **Standard Users.**
1. Selecteer **Bestand**  >  **Opslaan.**
1. Selecteer   >  **Inrichtingspakket exporteren** en selecteer vervolgens **Eigenaar**  >  **IT-beheerder.** Hiermee wordt de prioriteit van dit inrichtingspakket hoger dan de inrichtingspakketten die vanuit andere bronnen op dit apparaat worden toegepast.
1. Selecteer **Next**.
1. Selecteer op **de pagina Inrichtingspakketbeveiliging** een beveiligingsoptie.
   > [!IMPORTANT]  
   > Als u Pakket **ondertekenen inschakelen selecteert,** moet u ook een geldig certificaat selecteren dat u wilt gebruiken voor het ondertekenen van het pakket. Hiervoor selecteert u **Bladeren en** selecteert u het certificaat dat u wilt gebruiken om het pakket te ondertekenen.
...
- Windows Holographic, versie 20H2: als een apparaat een kioskconfiguratie heeft die een combinatie is van zowel globale toegewezen toegang als aan AAD-groepslid toegewezen toegang, ziet de gebruiker niets in het menu Start als het bepalen van het AAD-groepslidmaatschap mislukt.
