...
Zie AUMID's voor apps voor meer informatie over het inschakelen van uitgeschakelde functies of hoe spraakopdrachten werken met uitgeschakelde functies Cortana HoloLens [AUMIDs for apps](hololens-kiosk-reference.md#hololens-application-user-model-ids-aumids).
...
3. Voor de kioskmodus voor meerdere apps bepaalt u welke toepassingen moeten worden weergegeven in het menu Start. Voor elke toepassing is de [AUMID (Application User Model ID)](hololens-kiosk-reference.md#hololens-application-user-model-ids-aumids) nodig.
...
- De kioskmodus blokkeert niet dat een app (geconfigureerd als onderdeel van de kioskervaring) andere apps kan starten. Als u het starten van bepaalde apps/processen op HoloLens volledig wilt blokkeren, raadpleegt u Use Windows Defender Application Control on HoloLens 2 devices in Microsoft Intune - Azure (Windows Defender-toepassingsbeheer gebruiken op [HoloLens 2-apparaten in Microsoft Intune - Azure](/mem/intune/configuration/custom-profile-hololens)

## <a name="key-technical-considerations-for-kiosk-mode-for-hololens"></a>Belangrijke technische overwegingen voor kioskmodus voor HoloLens
...
### <a name="how-to-can-visitor-accounts-automatically-logon-into-kiosk-experience"></a>Hoe kunnen bezoekersaccounts zich automatisch bij de kioskervaring laten aanmeldingen?
...
De kioskmodus is alleen beschikbaar als het apparaat een Windows Holographic for Business. Alle HoloLens 2 worden met Windows Holographic for Business en er zijn geen andere edities. Op HoloLens 2 apparaat kan de kioskmodus out-of-the-box worden uitgevoerd.

HoloLens apparaten (1e generatie) moeten zowel op het gebied van de build van het besturingssysteem als de editie van het besturingssysteem worden bijgewerkt. Hier vindt u meer informatie over het bijwerken van een HoloLens (eerste generatie) naar [Windows Holographic for Business](hololens1-upgrade-enterprise.md) editie. Als u een HoloLens -apparaat (1e generatie) wilt bijwerken voor het gebruik van de kioskmodus, moet u er eerst voor zorgen dat op het apparaat Windows 10, versie 1803 of een latere versie wordt uitgevoerd. Als u het hulpprogramma Windows Device Recovery hebt gebruikt om uw HoloLens-apparaat (1e generatie) te herstellen naar de standaard build, of als u de meest recente updates hebt geïnstalleerd, kan uw apparaat worden geconfigureerd.
...
Stel het apparaat [HoloLens in om de Windows Apparaatportal.](/windows/mixed-reality/using-the-windows-device-portal#setting-up-hololens-to-use-windows-device-portal) De Apparaatportal is een webserver op uw HoloLens die u kunt verbinden vanuit een webbrowser op uw pc.

 > [!CAUTION]
 > Wanneer u de HoloLens voor het gebruik van Apparaatportal, moet u de ontwikkelaarsmodus op het apparaat inschakelen. Met de ontwikkelaarsmodus op een apparaat Windows Holographic for Business kunt u apps side-loaden. Deze instelling brengt echter het risico met zich mee dat een gebruiker apps kan installeren die niet zijn gecertificeerd door de Microsoft Store. Beheerders kunnen de mogelijkheid om de ontwikkelaarsmodus in te schakelen blokkeren met behulp van de instelling **ApplicationManagement/AllowDeveloper Unlock** in de [beleids-CSP](/windows/client-management/mdm/policy-configuration-service-provider). [Meer informatie over de ontwikkelaarsmodus.](/windows/uwp/get-started/enable-your-device-for-development#developer-mode)

De kioskmodus kan worden ingesteld via de REST API van Apparaatportal door een POST uit te voeren op /api/holographic/kioskmode/settings met één vereiste queryreeksparameter ('kioskModeEnabled' met de waarde 'true' of 'false') en één optionele parameter (startupApp) met de waarde van een pakketnaam. Houd er rekening mee dat Apparaatportal alleen bedoeld is voor ontwikkelaars en niet mag worden ingeschakeld op apparaten die geen ontwikkelaar zijn. De REST API is onderhevig aan wijzigingen in toekomstige updates/releases.

## <a name="troubleshooting"></a>Problemen oplossen

### <a name="issue---no-apps-are-shown-in-start-menu-in-kiosk-mode"></a>Probleem: worden er geen apps weergegeven in het menu Start in de kioskmodus?
...
- Controleer of AUMID van de app juist is opgegeven en geen versies bevat. Raadpleeg HoloLens [AUMID's](hololens-kiosk-reference.md#hololens-application-user-model-ids-aumids) voor Postvak IN-apps voor voorbeelden.
