...
1. Controleer of het Wi-Fi juiste instellingen heeft:
    - Configureer het EAP-type correct. Veelvoorkomende EAP-typen zijn EAP-TLS (13), EAP-TTLS (21) en PEAP (25).
    - Controleer de Wi-Fi SSID-naam en controleer of deze overeenkomt met de HEX-tekenreeks.
    - Zorg ervoor dat voor EAP-TLS TrustedRootCA de SHA-1-hash van het vertrouwde basis-CA-certificaat van de server bevat. Op Windows pc wordt met de opdracht 'certutil.exe -dump cert_file_name' de SHA-1-hash-tekenreeks van een certificaat weer gegeven.
2. Verzamel netwerkpakketopname op de logboeken van het toegangspunt, controller of AAA-server om erachter te komen waar de EAP-sessie mislukt.
    - Als de EAP-identiteit die wordt geleverd door HoloLens onverwacht is, controleert u of de identiteit correct is ingericht via Wi-Fi profiel of clientcertificaat.
    - Als de server het clientcertificaat HoloLens, controleert u of het vereiste clientcertificaat is ingericht op het apparaat.
    - Als HoloLens servercertificaat weigert, controleert u of het basis-CA-certificaat voor de server is ingericht op HoloLens.
3. Als het ondernemingsprofiel is ingericht via Wi-Fi inrichtingspakket, kunt u het inrichtingspakket toepassen op een Windows 10 pc. Als dit ook mislukt op Windows 10 pc, volgt u de handleiding voor Windows 802.1X-verificatie voor client.
...
1. Controleer of Wi-Fi is ingeschakeld. Controleer dit met behulp van de beweging Starten en selecteer Instellingen > Netwerkverbinding & Internet > Wi-Fi. Als Wi-Fi is, kunt u deze uitschakelen en opnieuw in- en uitschakelen.
2. Ga dichter bij de router of het toegangspunt zitten.
3. Start de Wi-Fi router opnieuw op en start HoloLens. Probeer opnieuw verbinding te maken.
...
Wanneer u zich op het apparaat bij een bedrijfs- of organisatieaccount aankeert, kan het ook Mobile Device Management-beleid (MDM) toepassen als het beleid is geconfigureerd door uw IT-beheerder.
...
1. Op uw pc: configureer Fiddler zodat externe computers verbinding kunnen maken.
    ...
    1. Klik op Opslaan.
3. Op uw HoloLens 2 u Fiddler configureren als proxyserver<sup>1:</sup>
    1. Open het Startmenu en selecteer Instellingen.
    1. Selecteer Network & Internet en vervolgens Proxy in het menu aan de linkerkant.
...
1. Wijzig in Handmatige installatie.
1. Voer het IP-adres in van de pc waarop Fiddler is geïnstalleerd.
1. Voer het hierboven genoteerde poortnummer in (de standaardwaarde is 8866).
1. Klik op Toepassen.
...
1. Op uw pc: exporteert u het Fiddler-certificaat.
    ...
    2. Klik op Fiddler-certificaat exporteren. Deze wordt op uw bureaublad op slaan.
...
2.  Importeer HoloLens 2 Fiddler-certificaat op uw HoloLens 2.
    1. Ga naar Instellingen ->->-certificaten.
    2. Klik op Certificaat installeren, blader naar de map Downloads en selecteer het Fiddler-certificaat.
    3. Wijzig Winkellocatie in Lokale computer.
    4. Wijzig Certificaatopslag in hoofdmap.
    5. Selecteer Installeren.
    6. Controleer of het certificaat wordt weergegeven in de lijst met certificaten. Als dat niet het volgende is, herhaalt u deze stappen.
...
Wireshark is een netwerkprotocolanalyse die wordt gebruikt om TCP/UDP-verkeer van en naar uw HoloLens 2 inspecteren. Hierdoor kunt u eenvoudig het verkeer identificeren dat het netwerk doorkruist naar uw HoloLens 2: hoeveel er is, de frequentie, de latentie tussen bepaalde hops, enzovoort.

#### <a name="prerequisites"></a>Vereisten:
- De pc moet internettoegang hebben en internet delen via Wi-Fi ondersteunen.

#### <a name="install-and-configure-wireshark"></a>Wireshark installeren en configureren
1. Installeer [Wireshark](https://www.wireshark.org/#download)op uw pc.
...
Wanneer een beïnvloed apparaat opnieuw contact probeert te maken met de Azure AD-tenant nadat het is verwijderd, mislukt de verificatie bij Azure AD. Dit effect is vaak onzichtbaar voor de gebruiker van het apparaat, omdat aanmelding in de cache via een pincode de gebruiker blijft toestaan zich aan te geven.

### <a name="mitigation"></a>Oplossing
Er is momenteel geen manier om een verwijderd apparaat HoloLens weer toe te voegen aan Azure AD. Betrokken apparaten moeten clean reflashed worden door de instructies te volgen voor [het reflashen van hun apparaat.](hololens-recovery.md#clean-reflash-the-device)

[Terug naar lijst](#list)

## <a name="autopilot-troubleshooting"></a>Problemen met Autopilot oplossen

De volgende artikelen kunnen een handige resource voor u zijn voor meer informatie en het oplossen van Autopilot-problemen. De artikelen zijn echter gebaseerd op Windows 10 Desktop en mogelijk is niet alle informatie van toepassing op HoloLens:
...
### <a name="what-logging-capabilities-does-hololens-2-offer"></a>Welke logboekmogelijkheden biedt HoloLens 2 bieden?
